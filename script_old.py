...
def ConfirmarOrden(orden):
    ...
    if len(orden) >= 3:
        ...
        texto_pedido += str(orden[len(orden) - 2].cantidad) + " " + orden[len(orden) - 2].nombreES + " y " + str(orden[len(orden) -2 ].cantidad) + " " + orden[len(orden) - 1].nombreES
...
def ProcesarOrden(transcripcion):
    ...
    for palabra in frase:
        if vigenciaCantidad > 0:
            if vigenciaCantidad < 2:
                cantidad = 1
            vigenciaCantidad = vigenciaCantidad - 1
        else:
